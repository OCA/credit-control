...
class ProductCategoryCredit(models.Model):
    ...
    type = fields.Selection([("customer", "Cliente"), ("supplier", "Proveedor")], required=True)
    used_credit = fields.Float("Monto utilizado", compute="_compute_used_credit", store=True)
    available_credit = fields.Float("Monto disponible", compute="_compute_available_credit", store=True)
        ...
        for rec in self:
            ...
            for inv in invoices:
                for line in inv.invoice_line_ids:
                    if rec.type == 'supplier' or (line.product_id.categ_id == rec.category_id):
    ...
    _sql_constraints = [
        ('unique_credit_per_category',
         ...
         'Solo puede existir una línea de crédito por categoría y tipo por contacto.')
    ]
